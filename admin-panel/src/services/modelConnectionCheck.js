...
export async function checkFirebaseMLConnection() {
  const checklist = [
    ...
    { id: 'ml-api-responding', label: 'ML API responding', status: 'pending' },
    { id: 'tflite-deployed', label: 'TFLite models deployed', status: 'pending' },
    { id: 'model-download', label: 'Model download URL accessible', status: 'pending' },
    { id: 'on-device-configured', label: 'On-device inference configured', status: 'pending' }
  ]
  ...
  try {
    ...
    if (mlResult.status === 'deployed') {
      ...
      checklist[3].status = 'pass'
      checklist[4].status = 'pass'
      details.modelCount = mlResult.details?.count || 0
      details.modelNames = mlResult.details?.models || []
      ...
      details.deploymentStatus = 'no models'
    } else {
      checklist[1].status = 'fail'
  ...
  return {
    ...
    name: 'Firebase ML',
    icon: 'model_training',
    description: 'Firebase Machine Learning for on-device TFLite model hosting. Enables hybrid inference with cloud API fallback.',
    checklist,
    details,
    overall: getOverallStatus(checklist)
  }
